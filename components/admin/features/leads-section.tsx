...
export function LeadsSection({ value, onChange, defaultOpen = false }: LeadsSectionProps) {
  ...
  const updateField = <K extends keyof LeadsFeatures>(field: K, newValue: LeadsFeatures[K]) => {
    onChange({ ...features, [field]: newValue })
  }

  return (
    <CollapsibleSection
      title="Lead Database Features"
      description="Built-in prospect database capabilities"
      icon="lucide/users"
      defaultOpen={defaultOpen}
      fieldCount={fieldCount - 1}
      filledCount={
        features._notApplicable ? 0 : filledCount - (features._notApplicable !== undefined ? 1 : 0)
      }

    ...
    <div className={cx("rounded-lg border bg-card overflow-hidden", className)}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          />
          <Icon name={icon as Parameters<typeof Icon>[0]["name"]} className="size-4 text-primary" />
  ...
  if (typeof value === "boolean") {
    return (
      <div className="flex items-center gap-1">
        <Icon name="lucide/check" className="size-4 text-green-600" />
    ...
    return (
      <div className="flex items-center gap-1">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className={cx("size-2.5 rounded-full", i < value ? "bg-primary" : "bg-muted")}
          />
        ))}

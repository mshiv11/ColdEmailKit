...
const getCategory = cache(async ({ params }: PageProps) => {
  ...
  if (!category || !integration) {
    ...
    if (categoryRedirect) {
      const url = `/categories/${categoryRedirect.destination.split("/").pop()}/integrations/${integrationSlug}`
...
const getMetadata = (category: CategoryOne, integration: IntegrationOne): Metadata => {
  ...
  return {
    title: `Top ${name} with ${integration.name} Integration`,
    description: `A curated collection of the top cold email ${lcFirst(name)} with ${integration.name} Integration. Find and compare tools to enhance your outreach.`,
  }
}

export const generateMetadata = async (props: PageProps): Promise<Metadata> => {
  const { category, integration } = await getCategory(props)
  const url = `/categories/${category.slug}/integrations/${integration.slug}`
    ...
    <>
      <Breadcrumbs
        items={[
          ...
          {
            href: `/categories/${category.slug}/integrations/${integration.slug}`,
      ...
      <Suspense fallback={<ToolListingSkeleton />}>
        <ToolQuery
          ...
          search={{
            placeholder: `Search ${category.label} with ${integration.name} Integration...`,
          }}
        />
      </Suspense>
    </>

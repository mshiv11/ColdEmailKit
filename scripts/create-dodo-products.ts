...
const API_KEY = isLive ? process.env.DODO_PAYMENTS_LIVE_API_KEY : process.env.DODO_PAYMENTS_API_KEY

if (!API_KEY || API_KEY === "dummy_dodo_key") {
  console.error(
    `❌ Error: Please set a valid ${isLive ? "DODO_PAYMENTS_LIVE_API_KEY" : "DODO_PAYMENTS_API_KEY"} in your .env file`,
  )
  console.log("\n📝 To get your API key:")
...
async function listProducts() {
  ...
  try {
    ...
    for await (const product of products) {
      console.log(`   - ${product.name} (${product.product_id}) - $${(product.price ?? 0) / 100}`)
    }
  } catch (error: any) {
